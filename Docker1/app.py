...
MODEL_WEIGHTS_PATH = 'mask_rcnn_damage_0010.h5'
load_weights(model, MODEL_WEIGHTS_PATH)

# Yolo model (side)
yolo_model_side = YoloModel("./scripts/best.pt")

# Yolo model predict (Damage)
yolo_model_dmg = YoloModel_dmg("./scripts/best_damage.pt")
...
def upload_image():
    ...
    if file:
        ...
        # Yolo model predict
        original, processed_side, coords = yolo_model_side.predict_single(image)

        # Yolo model predict (Damage)
        original_dmg, processed_dmg, coords_dmg = yolo_model_dmg.predict_single(image)
